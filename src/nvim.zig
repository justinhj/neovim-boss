...
pub const Nvim = struct {
    ...
    /// Get a high-level state snapshot of the Neovim editor.
    /// If brief is true, returns an orientation snapshot with active_window.
    /// If brief is false, returns all visible windows, marks, folds, selections, and diagnostics.
    pub fn getState(
        self: *Nvim,
        arena: std.mem.Allocator,
        brief: bool,
        target_buf: ?std.json.Value,
    ) !MsgPackObject {
        var lua_args: [2]MsgPackObject = undefined;
        lua_args[0] = .{ .boolean = brief };
        if (target_buf) |tb| {
            switch (tb) {
                .string => |s| lua_args[1] = .{ .string = try arena.dupe(u8, s) },
                .integer => |i| lua_args[1] = .{ .integer = i },
                else => lua_args[1] = .nil,
            }
        } else {
            lua_args[1] = .nil;
        ...
        return try self.execLua(arena, state_lua_script, &lua_args);
    }

    const state_lua_script = @embedFile("lua/get_state.lua");
    ...
    const lua_buf_helpers = @embedFile("lua/helpers.lua");
    const read_full_buf_lua_script = lua_buf_helpers ++ @embedFile("lua/read_full_buf.lua");
    const read_buf_range_lua_script = lua_buf_helpers ++ @embedFile("lua/read_buf_range.lua");
    const find_and_replace_buf_lua_script = lua_buf_helpers ++ @embedFile("lua/find_and_replace_buf.lua");
    const write_full_buf_lua_script = lua_buf_helpers ++ @embedFile("lua/write_full_buf.lua");
